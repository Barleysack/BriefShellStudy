...
if [ -e '/usr/bin/git' ]
	then
		echo "설치된 깃을 발견했습니다."
		git config --global user.name barleysack
		git config --global user.email hookypooky@naver.com
	else
		echo "깃을 설치합니다..."
...
if [ -e './.git' ] 
	then
		...
		git clone https://github.com/barleysack/BriefShellStudy
		mv ./git.sh ./BriefShellStudy/git.sh
